...
RUN CGO_ENABLED=0 go test ./... && \
    ...
    CGO_ENABLED=0 go build -trimpath -ldflags "-s -w -X github.com/zyvorai/nodra/internal/version.Version=${VERSION} -X github.com/zyvorai/nodra/internal/version.Commit=${COMMIT} -X github.com/zyvorai/nodra/internal/version.BuildDate=${BUILD_DATE}" -o /out/nodra-sim ./cmd/nodra-sim && \
    CGO_ENABLED=0 go build -trimpath -ldflags "-s -w -X github.com/zyvorai/nodra/internal/version.Version=${VERSION} -X github.com/zyvorai/nodra/internal/version.Commit=${COMMIT} -X github.com/zyvorai/nodra/internal/version.BuildDate=${BUILD_DATE}" -o /out/nodra-relay-bridge ./cmd/nodra-relay-bridge
...
COPY --from=build /out/nodra-sim /usr/local/bin/nodra-sim
COPY --from=build /out/nodra-relay-bridge /usr/local/bin/nodra-relay-bridge
