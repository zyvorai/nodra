...
2. **nodrad -> control plane**: bearer site credentials are supported; optional client certificates provide mTLS identity. `NODRA_REQUIRE_CLIENT_CERT` switches that listener to `RequireAndVerifyClientCert` when a client CA is configured. Console login and OIDC mint short-lived sessions (`NODRA_SESSION_TTL`, default 1h) via `/api/v1/auth/login` and `/api/v1/auth/oidc/*`; refresh with `POST /api/v1/auth/refresh` and revoke with `POST /api/v1/auth/logout`. The embedded console stores `expires_at` and refreshes before expiry. In file mode sessions persist under `<data-dir>/sessions.json` across restarts on that replica. With `NODRA_STORE=postgres`, sessions live in `nodra_console_sessions` and are shared across live replicas. Static `NODRA_ADMIN_TOKEN` / `NODRA_VIEWER_TOKEN` remain valid for automation. Custom roles (`POST /api/v1/roles`) mint additional write or read-only bearers: `roles.json` in file mode, or `nodra_custom_roles` in Postgres mode (also shared across replicas). `POST /api/v1/enrollment-token/rotate` replaces the bootstrap enrollment token; `NODRA_ENROLLMENT_TOKEN_TTL` can expire it. `POST /api/v1/ztp/bootstrap` and `nodrad ztp` enroll a site and write `nodrad.json` without a prior agent config. Console login and site enrollment each return 429 after five failures from the same address in five minutes.
